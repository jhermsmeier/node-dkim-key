...
Key.prototype = {
  ...
  parse( input ) {
    ...
    while( offset < value.length ) {
      ...
      let assign = value.indexOf( '=', offset + 1 )

      field = value.slice( offset, assign )

      if( !Key.keys.includes( field ) ) {
        throw new Error( `Unknown field name "${field}"` )
      }

      let delimiter = value.indexOf( ';', assign + 1 )

      fieldValue = value.slice( assign + 1, delimiter !== -1 ? delimiter : undefined )

      this[ Key.fieldMap[ field ] ] = field !== 'p' ?
        fieldValue : Buffer.from( fieldValue, 'base64' )

      offset = delimiter !== -1 ?
        delimiter + 1 : value.length
